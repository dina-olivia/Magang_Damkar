...
<body class="bg-gray-50 font-sans antialiased flex" x-data="{ modalTerbuka: false }">
    ...
    <aside class="w-64 bg-[#111827] text-gray-400 min-h-screen flex flex-col flex-shrink-0 border-r border-gray-800">
        ...
        <nav class="flex-1 p-3 space-y-1 text-xs overflow-y-auto"
            x-data="{ menuKejadian: false, menuOperasional: true, menuPersonil: false }">

            <a href="../index.php"
                class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg hover:bg-gray-800 hover:text-white transition">
                <i class="bi bi-speedometer2 text-base"></i> Dashboard
            </a>

            <div>
                <button @click="menuKejadian = !menuKejadian"
                    class="w-full flex items-center justify-between px-3 py-2.5 rounded-lg hover:bg-gray-800 hover:text-white transition">
            ...
            <div>
                <button @click="menuOperasional = !menuOperasional"
                    class="w-full flex items-center justify-between px-3 py-2.5 rounded-lg hover:bg-gray-800 hover:text-white transition">
                    <span class="flex items-center gap-2.5">
                ...
                <div x-show="menuOperasional" class="pl-8 mt-1 space-y-1 bg-gray-900/40 rounded-lg py-1">
                    <a href="penugasan.php"
                        class="block px-3 py-2 text-white bg-blue-600/20 font-semibold rounded-md">Penugasan Tim</a>
            ...
            <div>
                <button @click="menuPersonil = !menuPersonil"
                    class="w-full flex items-center justify-between px-3 py-2.5 rounded-lg hover:bg-gray-800 hover:text-white transition">
                    <span class="flex items-center gap-2.5">
            ...
            <a href="../logout.php"
                class="flex items-center gap-2.5 px-3 py-2.5 text-red-400 rounded-lg hover:bg-red-950/30 transition">
    ...
    <main class="flex-1 p-6 min-h-screen overflow-x-hidden">
        ...
        <div class="flex justify-end items-center mb-6 gap-4">
            ...
            <div class="flex items-center gap-2 border-l pl-4 border-gray-200">
                <div class="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center text-gray-500 font-bold">
        ...
        <div class="bg-white border border-blue-600 rounded-2xl p-4 flex justify-between items-center shadow-sm mb-8">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <span
                            class="bg-green-100 text-green-700 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide">On
                            Duty</span>
                    </div>
            ...
            <div class="flex gap-8 text-right pr-4">
                <div>
                    ...
                    <p class="text-lg font-bold text-blue-600">3 <span class="text-gray-400 font-normal text-xs">/
                            15</span></p>
                    <p class="text-[9px] text-gray-400">Ready</p>
                </div>
                <div>
                    <p class="text-[10px] font-bold text-gray-400 tracking-wider">ARMADA</p>
                    <p class="text-lg font-bold text-orange-500">1 <span class="text-gray-400 font-normal text-xs">/
                            4</span></p>
                    <p class="text-[9px] text-gray-400">Ready</p>
                </div>
            </div>
        ...
        <div class="flex gap-4 mb-4">
            <div class="relative flex-1">
                ...
                <input type="text" placeholder="Cari No. SPT atau Alamat Kejadian..."
                    class="w-full bg-white border border-gray-100 rounded-xl pl-11 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20">
            </div>
            <select
                class="bg-white border border-gray-100 rounded-xl px-4 py-2.5 text-sm text-gray-600 focus:outline-none w-48">
                ...
                <tbody class="text-sm text-gray-700 divide-y divide-gray-50">
                    ...
                    <?php while ($row = mysqli_fetch_assoc($tampil_spt)): ?>
                        <tr class="hover:bg-gray-50/50 transition">
                            <td class="p-4">
                                <span
                                    class="bg-blue-50 text-blue-600 font-bold px-2.5 py-1 rounded-md text-xs border border-blue-100">
                                    <?= $row['nomor_spt']; ?>
                                </span>
                            </td>
                            <td class="p-4">
                                ...
                                <p class="text-xs text-gray-400 mt-0.5">
                                    <?= ucfirst($row['jenis_kejadian']); ?> <span class="text-gray-300 mx-1">|</span> Ref:
                                    <?= $row['nomor_laporan']; ?>
                                </p>
                            </td>
                            <td class="p-4">
                                <div class="text-xs text-gray-600 space-y-0.5">
                                    <p class="flex items-center gap-1"><i class="bi bi-people text-gray-400"></i> 4 Petugas
                                        (Danru: A. Supriadi)</p>
                                    <p class="flex items-center gap-1"><i class="bi bi-truck text-gray-400"></i> 1 Armada
                                        Unit (🚒 <?= $row['nama_regu']; ?>)</p>
                                </div>
                            </td>
                            ...
                            <td class="p-4 whitespace-nowrap">
                                <span
                                    class="px-2.5 py-1 rounded-full text-xs font-semibold <?= $row['status'] == 'berangkat' ? 'bg-blue-50 text-blue-600 border border-blue-100' : 'bg-gray-100 text-gray-600' ?>">
                                    <?= $row['status']; ?>
                                </span>
                            </td>
                            <td class="p-4">
                                <div class="flex items-center justify-center gap-3">
                                    <button class="text-gray-400 hover:text-gray-600 text-base"
                                        title="Cetak Surat Perintah">
                                        <i class="bi bi-printer"></i>
                                    </button>
                                    <button
                                        class="bg-gray-950 hover:bg-gray-800 text-white text-xs font-medium px-3 py-1.5 rounded-lg shadow-sm tracking-wide transition">
                                        Monitor
                                    </button>
                                </div>
                            </td>
        ...
        <div x-show="modalTerbuka" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" x-cloak
            x-transition>
            <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl" @click.away="modalTerbuka = false">
                ...
                <form action="penugasan.php" method="POST">
                    <div class="mb-4">
                        ...
                        <select name="laporan_kejadian_id"
                            class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                            required>
                    ...
                    <div class="flex justify-end gap-3">
                        <button type="button" @click="modalTerbuka = false"
                            class="px-4 py-2 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50">Batal</button>
                        <button type="submit" name="kirim_tim"
                            class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-medium shadow-sm">Kirim
                            Tim</button>
                    </div>
                </form>
            </div>
