...
// Ambil data personil untuk pilihan Komandan Regu (bisa disesuaikan dengan role/jabatan di dinas)
$query_personil = mysqli_query($conn, "SELECT id, nama, jabatan FROM personil WHERE status = 'aktif' ORDER BY nama ASC");

// 2. Proses ketika Form Penugasan dikirim (Submit)
if (isset($_POST['ganti_status_proses'])) {
    $komandan_id = intval($_POST['komandan_id']);
    $waktu_berangkat = $_POST['waktu_berangkat']; // Mengambil jam berangkat tim

    // Mulai Database Transaction agar jika salah satu query gagal, data tidak rusak
    ...
    try {
        ...
        // B. Insert data penugasan baru ke tabel penugasan (sesuai struktur Word)
        $status_tugas = 'dalam penanganan';
        $stmt_insert = mysqli_prepare($conn, "INSERT INTO penugasan (laporan_id, waktu_berangkat, komandan_id, status) VALUES (?, ?, ?, ?)");
        ...
        header("Location: laporan_kejadian.php?error=Gagal memproses laporan: " . $e->getMessage());
        exit;
    }
}
?>
            ...
            <div class="card border-0 shadow-sm rounded-4 card-detail p-4 mb-4 bg-white">
                ...
                <table class="table table-sm table-borderless m-0 text-muted">
                    ...
                    <tr>
                        ...
                        <td class="text-uppercase text-danger fw-bold">: <?= htmlspecialchars($data_laporan['jenis_kejadian']) ?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>: <?= htmlspecialchars($data_laporan['pelapor']) ?> (<?= htmlspecialchars($data_laporan['no_hp']) ?>)</td>
                    </tr>
                    <tr>
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="komandan_id" class="form-select" required>
                                ...
                                <?php while($pers = mysqli_fetch_assoc($query_personil)): ?>
                                    <option value="<?= $pers['id'] ?>">
                                        <?= htmlspecialchars($pers['nama']) ?> (<?= htmlspecialchars($pers['jabatan']) ?>)
                                    </option>
                                <?php endwith; ?>
                            </select>
                        ...
                        <div class="col-12 text-end mt-4">
                            <hr class="text-muted">
                            <button type="submit" name="ganti_status_proses" class="btn btn-warning px-5 fw-bold text-dark shadow-sm">
