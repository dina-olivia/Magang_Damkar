...
<head>
    ...
    <style>
        ...
        .content-card-wrapper {
            background: white;
            border-radius: 25px;
            /* Membuat sudut kotak putih melengkung */
            overflow: hidden;
            /* Memastikan header tidak keluar jalur lengkungan */
        ...
        .custom-header-red {
            ...
            /* Lengkungan khas di bagian bawah header */
            margin-bottom: 20px;
        }
        ...
        .box-proses {
            background: linear-gradient(45deg, #ffc107, #e6ac00);
            color: #212529;
        }

        .box-selesai {
            background: linear-gradient(45deg, #198754, #105a38);
        }
                ...
                <div class="row g-4 mb-5">
                    <div class="col-md-4">
                        <div class="card-stat box-masuk">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <p class="m-0 small">Laporan Masuk</p>
                                    <h3 class="fw-bold m-0">
                                        <?= mysqli_num_rows(mysqli_query($conn, "SELECT * FROM laporan_kejadian WHERE status='Masuk'")) ?>
                                    </h3>
                                </div>
                    ...
                    <div class="col-md-4">
                        <div class="card-stat box-proses">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <p class="m-0 small">Dalam Proses</p>
                                    <h3 class="fw-bold m-0">
                                        <?= mysqli_num_rows(mysqli_query($conn, "SELECT * FROM laporan_kejadian WHERE status='Proses'")) ?>
                                    </h3>
                                </div>
                    ...
                    <div class="col-md-4">
                        <div class="card-stat box-selesai">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <p class="m-0 small">Laporan Selesai</p>
                                    <h3 class="fw-bold m-0">
                                        <?= mysqli_num_rows(mysqli_query($conn, "SELECT * FROM laporan_kejadian WHERE status='Selesai'")) ?>
                                    </h3>
                                </div>
                            ...
                            <tbody>
                                ...
                                while ($row = mysqli_fetch_assoc($res)) {
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span
                                                class="fw-bold text-dark d-block"><?= strtoupper($row['jenis_kejadian'] ?? 'RESCUE') ?></span>
                                            <span class="small text-muted"><i
                                                    class="bi bi-clock me-1"></i><?= date('d M Y', strtotime($row['tanggal'])) ?></span>
                                        </td>
                                        <td>
                                            <span class="small text-dark fw-semibold"><i
                                                    class="bi bi-geo-alt-fill text-danger me-1"></i><?= $row['lokasi'] ?? '-' ?></span>
                                        </td>
                                        <td>
                                            <span class="d-inline-block text-truncate small text-muted"
                                                style="max-width: 150px;" title="<?= $row['deskripsi'] ?>">
                                                <?= $row['deskripsi'] ?? '-' ?>
                                            </span>
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-center">
                                            <?php
                                            $ver = strtolower($row['verifikasi'] ?? '');
                                            if ($ver === 'valid') {
                                                echo '<span class="badge bg-success small">Terverifikasi</span>';
                                            } elseif ($ver === 'palsu') {
                                                echo '<span class="badge bg-danger small">Ditolak</span>';
                                            } else {
                                                echo '<span class="badge bg-secondary small">Belum Verifikasi</span>';
                                            }
                                            ?>
                                        </td>
                                        <td class="text-center">
                                            <span class="badge rounded-pill <?= $cls ?> px-3 py-2 shadow-sm"
                                        ...
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center gap-1">
                                                <a href="detail_kejadian.php?no_lp=<?= $row['nomor_laporan'] ?>"
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu shadow border-0">
                                                        <li><a class="dropdown-item small"
                                                                href="?update_status=Masuk&no_lp=<?= $row['nomor_laporan'] ?>">Masuk</a>
                                                        </li>
                                                        <li><a class="dropdown-item small"
                                                                href="?update_status=Proses&no_lp=<?= $row['nomor_laporan'] ?>">Proses</a>
                                                        </li>
                                                        <li><a class="dropdown-item small"
                                                                href="?update_status=Selesai&no_lp=<?= $row['nomor_laporan'] ?>">Selesai</a>
                                                        </li>
                                                    </ul>
                                                </div>

                                                <a href="?hapus=<?= $row['nomor_laporan'] ?>"
